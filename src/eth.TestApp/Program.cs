            ...
            foreach (var space in bot.GetSpaceInitializers())            
                switch (space.Key)
                {
                    case 1:
                        ...
                        break;
                    case 2:
                        var replyWithExceptionPlugin2 = new ReplyWithExceptionPlugin();

                        space.Value.Plugins.Enqueue(new HealthStatusPlugin());
                        space.Value.Plugins.Enqueue(new HogwartsPlugin());

                        space.Value.Plugins.Enqueue(replyWithExceptionPlugin2);

                        space.Value.HealthListeners.Enqueue(replyWithExceptionPlugin2);
                        break;
                }
